...
if modalita == "Amministrazione":
        ...
        if lista_clienti:
            ...
            if st.button("Crea Progetto"):
                if nome_nuovo_progetto:
                    ...
                    # Creazione variante base di default per il progetto
    ...
    # 2. Tendina Selezione Progetto (filtrata per il cliente scelto)
    ...
    # ==========================================
    # SCHEDA 1: CLIENTE E PARAMETRI ECONOMICI
    # ==========================================
    with st.expander("👤 Scheda Cliente e Parametri Economici", expanded=False):
        col_cli1, col_cli2 = st.columns(2)
        cliente_nome_mod = col_cli1.text_input("Nome / Ragione Sociale Cliente", value=cliente_attuale["ragione_sociale"])
        cliente_ind_mod = col_cli2.text_input("Indirizzo / Cantiere", value=cliente_attuale.get("indirizzo") or "")
        
        st.markdown("---")
        st.write("**Modifica Dati Cliente:**")
    ...
    # ==========================================
    # SELEZIONE E GESTIONE VARIANTI FINITURA (SOPRA L'EXPANDER FINITURE)
    # ==========================================
    ...
    # ==========================================
    # SCHEDA 2: FINITURE PREDEFINITE VARIANTE
    # ==========================================
    with st.expander(f"🎨 Finiture Predefinite Variante ({variante_attuale['nome_variante']})", expanded=False):
        ...
        if st.button("💾 Salva Impostazioni e Finiture Variante"):
            ...
            st.success("Parametri e finiture predefinite della variante salvati!")
            st.rerun()
    ...
    tab_ins1, tab_ins2 = st.tabs(["⚡ Carica Modulo Standard", "✏️ Crea Modulo Su Misura"])
    ...
    with tab_ins1:
        ...
        if moduli_std_db:
            ...
            amb_std_dest = col_s1.selectbox("Seleziona Ambiente Destinazione", options=[a['id'] for a in ambienti], format_func=lambda x: [a['nome_ambiente'] for a in ambienti if a['id']==x][0], key="amb_std")
            mod_std_sel_id = col_s2.selectbox("Seleziona Modulo Standard", options=[m['id'] for m in moduli_std_db], format_func=lambda x: [m['nome_modulo'] for m in moduli_std_db if m['id']==x][0])
            
            m_std_obj = next(m for m in moduli_std_db if m['id'] == mod_std_sel_id)
            st.info(f"**Dettagli Modulo:** Categoria: `{m_std_obj['categoria']}` | Misure: `{m_std_obj['larghezza_mm']}x{m_std_obj['altezza_mm']}x{m_std_obj['profondita_mm']} mm` | Apertura: `{m_std_obj['tipo_apertura']}`")
            ...
            st.warning("Nessun modulo standard presente nel catalogo. Configurali nell'area Amministrazione.")

    # TAB 2: CREA MODULO CUSTOM
    with tab_ins2:
        amb_m = st.selectbox("Seleziona Ambiente Destinazione", options=[a['id'] for a in ambienti], format_func=lambda x: [a['nome_ambiente'] for a in ambienti if a['id']==x][0], key="amb_manual")
        c1, c2, c3 = st.columns(3)
        categoria = c1.selectbox("Categoria Modulo", ["basi", "pensili", "colonne", "mensole", "zoccoli", "accessori"])
    ...
    if righe_preventivo:
        ...
        for amb_nome in df_prev["Ambiente"].unique():
            ...
            st.dataframe(df_amb[["Categoria", "Modulo", "Dimensioni", "Finitura Anta", "costo_industriale", "Prezzo Ricaricato (€)"]], use_container_width=True)
        ...
        pdf_data = genera_pdf_preventivo(
            ...
            indirizzo=cliente_attuale.get("indirizzo", ""),
            sconto_fin=sconto_f,
            ricarico_mont=mont_p,
            ricarico_proj=ricarico_p
        )
